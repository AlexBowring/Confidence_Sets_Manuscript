...
for t=1:nRlz
    fprintf('.');
      for i=1:nSubj
	    ...
	    %
        Noise = create_noise(wdim, 'ramp', var, smo, trnind);
      ...
      observed_resid_std   = reshape(...
         biasmystd(cohen_resid,stdblk),...
           dim);
      standardized_cohen_resid = spdiags(1./reshape(observed_resid_std, [prod(dim) 1]), 0,prod(dim),prod(dim))*cohen_resid;
