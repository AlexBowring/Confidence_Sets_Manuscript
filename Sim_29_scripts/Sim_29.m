function Sim_29(nSubj,SvNm,nRlz)
...
Sig = create_signal(wdim, 'multi', mag, smo, trnind);
...
cohen_d = Sig./non_stationary_var;
...
for t=1:nRlz
    ...
    observed_std  = zeros(prod(dim),1);
      ...
      observed_resid_mean = zeros(prod(dim), 1);
      observed_resid_std  = zeros(prod(dim), 1);
      
      for i=1:nSubj
          cohen_resid = create_resid(observed_data(:,i), observed_mean, observed_std, 2); 
          observed_resid_mean = observed_resid_mean + cohen_resid;
          observed_resid_std  = observed_resid_std + cohen_resid.^2;
      end
      
      observed_resid_mean = observed_resid_mean/nSubj;
      observed_resid_std  = sqrt(observed_resid_std/nSubj - observed_resid_mean.^2);
      observed_resid_std   = reshape(observed_resid_std, dim);
      
      for i=1:nSubj
          cohen_resid = create_resid(observed_data(:,i), observed_mean, observed_std, 2); 
          subject_resid_field                 = reshape(cohen_resid, [dim 1]);
          subject_resid_field                 = subject_resid_field./observed_resid_std;
    ...
    lower_contour_raw_80             = observed_cohen_d >= (1 - (3/(4*nSubj - 5)))^(-1)*thr - supGa_raw_80*tau*observed_resid_std;
    upper_contour_raw_80             = observed_cohen_d >= (1 - (3/(4*nSubj - 5)))^(-1)*thr + supGa_raw_80*tau*observed_resid_std;
    ...
    lower_contour_raw_90             = observed_cohen_d >= (1 - (3/(4*nSubj - 5)))^(-1)*thr - supGa_raw_90*tau*observed_resid_std;
    upper_contour_raw_90             = observed_cohen_d >= (1 - (3/(4*nSubj - 5)))^(-1)*thr + supGa_raw_90*tau*observed_resid_std;
    ...
    lower_contour_raw_95             = observed_cohen_d >= (1 - (3/(4*nSubj - 5)))^(-1)*thr - supGa_raw_95*tau*observed_resid_std;
    upper_contour_raw_95             = observed_cohen_d >= (1 - (3/(4*nSubj - 5)))^(-1)*thr + supGa_raw_95*tau*observed_resid_std;
    ...
    lower_contour_observed_80             = observed_cohen_d >= (1 - (3/(4*nSubj - 5)))^(-1)*thr - supGa_observed_80*tau*observed_resid_std;
    upper_contour_observed_80             = observed_cohen_d >= (1 - (3/(4*nSubj - 5)))^(-1)*thr + supGa_observed_80*tau*observed_resid_std;
    ...
    lower_contour_observed_90             = observed_cohen_d >= (1 - (3/(4*nSubj - 5)))^(-1)*thr - supGa_observed_90*tau*observed_resid_std;
    upper_contour_observed_90             = observed_cohen_d >= (1 - (3/(4*nSubj - 5)))^(-1)*thr + supGa_observed_90*tau*observed_resid_std;
    ...
    lower_contour_observed_95             = observed_cohen_d >= (1 - (3/(4*nSubj - 5)))^(-1)*thr - supGa_observed_95*tau*observed_resid_std;
    upper_contour_observed_95             = observed_cohen_d >= (1 - (3/(4*nSubj - 5)))^(-1)*thr + supGa_observed_95*tau*observed_resid_std;
    ...
    lower_condition_80 = (1 - (3/(4*nSubj - 5)))^(-1)*thr - supGa_raw_80*tau*observed_resid_std;
    upper_condition_80 = (1 - (3/(4*nSubj - 5)))^(-1)*thr + supGa_raw_80*tau*observed_resid_std;
    lower_condition_90 = (1 - (3/(4*nSubj - 5)))^(-1)*thr - supGa_raw_90*tau*observed_resid_std;
    upper_condition_90 = (1 - (3/(4*nSubj - 5)))^(-1)*thr + supGa_raw_90*tau*observed_resid_std;
    lower_condition_95 = (1 - (3/(4*nSubj - 5)))^(-1)*thr - supGa_raw_95*tau*observed_resid_std;
    upper_condition_95 = (1 - (3/(4*nSubj - 5)))^(-1)*thr + supGa_raw_95*tau*observed_resid_std;
    
    lower_condition_80_boundary_values = getBdryvalues(lower_condition_80, cohen_d_boundary_edges);
    upper_condition_80_boundary_values = getBdryvalues(upper_condition_80, cohen_d_boundary_edges);

    lower_condition_90_boundary_values = getBdryvalues(lower_condition_90, cohen_d_boundary_edges);
    upper_condition_90_boundary_values = getBdryvalues(upper_condition_90, cohen_d_boundary_edges);
    
    lower_condition_95_boundary_values = getBdryvalues(lower_condition_95, cohen_d_boundary_edges);
    upper_condition_95_boundary_values = getBdryvalues(upper_condition_95, cohen_d_boundary_edges);
    
    observed_cohen_d_true_boundary_values = getBdryvalues(observed_cohen_d, cohen_d_boundary_edges);
    ...
    lower_condition_80_observed = (1 - (3/(4*nSubj - 5)))^(-1)*thr - supGa_observed_80*tau*observed_resid_std;
    upper_condition_80_observed = (1 - (3/(4*nSubj - 5)))^(-1)*thr + supGa_observed_80*tau*observed_resid_std;
    lower_condition_90_observed = (1 - (3/(4*nSubj - 5)))^(-1)*thr - supGa_observed_90*tau*observed_resid_std;
    upper_condition_90_observed = (1 - (3/(4*nSubj - 5)))^(-1)*thr + supGa_observed_90*tau*observed_resid_std;
    lower_condition_95_observed = (1 - (3/(4*nSubj - 5)))^(-1)*thr - supGa_observed_95*tau*observed_resid_std;
    upper_condition_95_observed = (1 - (3/(4*nSubj - 5)))^(-1)*thr + supGa_observed_95*tau*observed_resid_std;
    
    lower_condition_80_observed_boundary_values = getBdryvalues(lower_condition_80_observed, cohen_d_boundary_edges);
    upper_condition_80_observed_boundary_values = getBdryvalues(upper_condition_80_observed, cohen_d_boundary_edges);
    
    lower_condition_90_observed_boundary_values = getBdryvalues(lower_condition_90_observed, cohen_d_boundary_edges);
    upper_condition_90_observed_boundary_values = getBdryvalues(upper_condition_90_observed, cohen_d_boundary_edges);
    
    lower_condition_95_observed_boundary_values = getBdryvalues(lower_condition_95_observed, cohen_d_boundary_edges);
    upper_condition_95_observed_boundary_values = getBdryvalues(upper_condition_95_observed, cohen_d_boundary_edges);
